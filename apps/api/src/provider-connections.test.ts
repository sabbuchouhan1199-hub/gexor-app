...
test("workspace provider connection lifecycle is isolated, audited, and redacted", async()=>{
 ...
 assert.equal(repository.listProviders().length,2); assert.equal(repository.listModels("ollama").length,1);
...
test("provider connection HTTP lifecycle requires bearer ownership and never returns references", async()=>{
 ...
 const register=async(email:string)=>{const response=await app.inject({method:"POST",url:"/api/v1/auth/register",payload:{displayName:"Owner",email,password:"valid-passphrase"}}); assert.equal(response.statusCode,201); return response.json() as {sessionToken:string;workspace:{workspaceId:string}}};
 ...
 assert.equal((await app.inject({method:"POST",url,headers:{authorization:`Bearer ${other.sessionToken}`,"x-workspace-id":other.workspace.workspaceId},payload:{providerKey:"ollama",credentialReference:"vault:owner/ref"}})).statusCode,404);
 const connected=await app.inject({method:"POST",url,headers:{authorization:`Bearer ${owner.sessionToken}`,"x-workspace-id":owner.workspace.workspaceId},payload:{providerKey:"ollama",credentialReference:"vault:owner/ref"}});
 ...
 const headers={authorization:`Bearer ${owner.sessionToken}`,"x-workspace-id":owner.workspace.workspaceId};
